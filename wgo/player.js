...
Player.prototype = {
	...
	dispatchEvent: function(evt) {
		...
		for(var i=0;i<this.listeners[evt.type].length;i++){
...
//		}
    ...
    remove: function() {
        ...
        try {
            this.kifuReader.previous();
            var node=this.kifuReader.node
            this.kifuReader.node=node.children[node._last_selected].remove();
            this.update();
        }
        catch(err) {
            this.error(err);
